...
function MatchTable() {
	...
	const dataMatch =
		data &&
		data.football.map(function (item, index) {
			return (
				<tr
					key={item.uuid}
				...
				>
					<td className="col-4">
						<div className="d-flex justify-content-around ">{item.name}</div>
					</td>
	...
	return (
		<div className="container-fluid m-0 p-0 px-0 mt-4 d-flex flex-column align-items-center justify-content-center">
			<div className="row col-12 col-lg-12 col-md-12 col-sm-12 d-flex justify-content-between align-items-center flex-row-reverse">
				<div className="head-table col-3">
					<p>جدول الدوري السوري</p>
				</div>
				<div
					className="burger-button col-1 m-4"
			...
			<div className="row col-12 col-lg-12 col-md-12 col-sm-12">
				<div
					className="table1 col-12 "
					style={{ display: tableDisplay ? "none" : "flex" }}
				>
					<table className="col-12">
						...
						<tbody className=" col-12 ">{dataMatch}</tbody>
					</table>
				</div>
			</div>
		</div>
